...
class NerEmbeddiaProcessor(DataProcessor):
    ...
    def _read_conllu(self, path):
        ...
        for sentence in sentences:
            ...
            for token in sentence:
                ...
                tags.append(list(token['misc'].keys())[0].upper())
                misc.append({})
    ...
    def get_train_examples(self, data_dir, cv_part, folds, partial_train_data_usage=1.0, read_conllu=False):
        """See base class."""
        if read_conllu:
            data = self._read_conllu(os.path.join(data_dir, "train_ner.conllu"))
    ...
    def get_dev_examples(self, data_dir, cv_part, read_conllu=False):
        """See base class."""
        if read_conllu:
            data = self._read_conllu(os.path.join(data_dir, "test_ner.conllu"))
...
def main():
    def bert_cross_validation_iteration(cross_validation_part, partial_train_data_usage):
        ...
        label_list = processor.get_labels(args.conllu)
        ...
        if args.do_train:
            train_examples = processor.get_train_examples(args.data_dir, cross_validation_part, args.folds, partial_train_data_usage, read_conllu=args.conllu)
        ...
        if args.do_train:
            ...
            for _ in trange(int(args.num_train_epochs), desc="Epoch"):
                ...
                if args.do_eval_in_training:
                    eval_examples = processor.get_dev_examples(args.data_dir, cross_validation_part, read_conllu=args.conllu)
