...
class BERTDataset(Dataset):
    ...
    def count_different_tokens(self):
        dict = {}
        p = 1
        for i in range(self.corpus_lines):
            cur_id = self.sample_counter
            self.sample_counter += 1
            if not self.on_memory:
                # after one epoch we start again from beginning of file
            ...
            for el in tokens_b:
                if el in dict:
                    ...
                    dict[el] = 0
            if float(i) * 100 / self.corpus_lines > p:
                print(str(i) + ' lines processed - ' + str(p) + ' %')
                p += 1
